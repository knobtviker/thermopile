package com.knobtviker.thermopile.presentation.utils.controllers;
...
public class PIDController {
    ...
    private int highLimit = 35;            // assuming control of motor speed and thereby max would be 900 deg/sec
    private int lowLimit = 5; // -highLimit;
    private int previous_error = 0;
    private int deadband = 1; //0;
    ...
    public int doPID(int processVariable) {
        ...
        // log data
        if (cycleCount > 2) {
            Timber.d("SP: %s MV: %s PV %s Integral %s Kp*Error %s Kd*Derivative %s Error %s dt %s",
                setpoint, outputMV, processVariable, integral, Kp * error, Kd * derivative, error, dt);
        }
    ...
    private void msDelay(long period) {
        if (period <= 0) {
            return;
        }

        final long end = System.currentTimeMillis() + period;
