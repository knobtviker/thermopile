...
public class Bme680 implements AutoCloseable {
    ...
    private void connect(I2cDevice device) throws IOException {
        ...
        setTemperatureOversample(OVERSAMPLING_16X);
        setHumidityOversample(OVERSAMPLING_16X);
        setPressureOversample(OVERSAMPLING_16X);
    ...
    private void getSensorData() throws IOException {
        ...
        if ((status & BME680_NEW_DATA_MASK) == 0) {
            ...
            Log.i(TAG, data.pressure + " --- " + pressure + " --- " + ((buffer[2] & 0xff) << 12) + " --- " + ((buffer[3] & 0xff) << 4) + " --- " + ((buffer[4] & 0xff) >> 4) + " --- "+buffer[2] +" --- "+buffer[3]+" --- "+buffer[4]);
        }
    }
