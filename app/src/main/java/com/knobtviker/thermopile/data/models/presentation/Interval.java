...
public final class Interval implements Serializable {
     ...
     *
     * @param other  the interval to examine
     * @return the gap interval, null if no gap
     */
    @Nullable
    public Interval gap(@NonNull Interval other) {
        Objects.requireNonNull(other, "other");
        final long otherStart = other.getStart().toEpochMilli();
        final long otherEnd = other.getEnd().toEpochMilli();
        final long thisStart = getStart().toEpochMilli();
        final long thisEnd = getEnd().toEpochMilli();
        if (thisStart > otherEnd) {
            return new Interval(other.getEnd(), getStart());
        } else if (otherStart > thisEnd) {
            return new Interval(getEnd(), other.getStart());
        } else {
            return null;
        }
    }
